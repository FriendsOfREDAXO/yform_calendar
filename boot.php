    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.btn-show-occurrences-list')) {
            ...
            .then(data => {
                let html = '';
                if (data.error) {
                    html = '<div class="alert alert-danger"><strong>Fehler:</strong> ' + data.error + '</div>';
                } else if (!data.occurrences || data.occurrences.length === 0) {
                    html = '<p class="text-muted">Keine Termine gefunden.</p>';
                } else {
